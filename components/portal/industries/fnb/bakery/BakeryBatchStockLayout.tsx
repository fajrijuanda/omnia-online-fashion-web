...
import { Factory, RefreshCcw, Search, ClipboardList, BarChart3 } from "lucide-react";
...
export function BakeryBatchStockLayout({ subIndustryName = "Bakery" }: { subIndustryName?: string }) {
  ...
  const rows = useMemo(() => {
    return catalog.products.map((product, index) => ({
      ...
      cells: [
        ...
        product.name,
        catalog.categories.find((category) => category.id === product.categoryId)?.name ?? "Kategori",
        `${24 + index * 8} pcs`,
        `${6 + index} pcs`,
        index % 2 ? "Display" : "Produksi"
      ]
    }));
  }, [catalog]);
  ...
  const headers = ["Batch", "Produk", "Kategori", "Produksi", "Display", "Status"];

  const kpis = [
    { label: "Batch aktif", value: String(filteredRows.length), icon: Factory, delta: "+15%", caption: "vs minggu lalu" },
    { label: "Display stock", value: "24", icon: BarChart3, delta: "+8.2%", caption: "vs minggu lalu" },
    { label: "Total produk", value: String(catalog.products.length), icon: ClipboardList, delta: "Aktif", caption: "hari ini" },
  ];
    ...
    <div className="min-h-full space-y-4 bg-[#fffaf5] p-3 pb-4 sm:p-5 lg:p-6">
      <section className="rounded-[18px] border border-slate-100 bg-white p-4 shadow-sm sm:rounded-[28px] sm:p-6">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--portal-primary)]">Bakery production</p>
            <h1 className="mt-1.5 text-xl font-black text-[#172033] sm:mt-2 sm:text-3xl">Batch Stock</h1>
            <p className="mt-1 max-w-3xl text-xs font-bold leading-5 text-slate-500 sm:mt-2 sm:text-sm sm:leading-6">{subIndustryName} - Pantau batch produksi, display stock, dan tanggal produksi produk bakery.</p>
          </div>
