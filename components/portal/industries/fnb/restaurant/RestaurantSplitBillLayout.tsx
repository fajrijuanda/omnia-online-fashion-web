...
import { CreditCard, RefreshCcw, Search, ClipboardList, BarChart3 } from "lucide-react";
...
export function RestaurantSplitBillLayout({ subIndustryName = "Restoran" }: { subIndustryName?: string }) {
  ...
  const rows = useMemo(() => {
    return orders.map((order) => ({
      ...
      cells: [
        order.invoiceNumber,
        order.status,
        order.items.map((item) => `${item.quantity}x ${item.product?.name ?? "Menu"}`).join(", "),
        rupiah(order.totalAmount),
        "2 tamu",
        rupiah(order.totalAmount / 2),
      ]
    }));
  }, [orders]);
  ...
  const headers = ["Invoice", "Status", "Item", "Total", "Split", "Per Tamu"];

  const kpis = [
    { label: "Tagihan aktif", value: String(filteredRows.length), icon: CreditCard, delta: "+15%", caption: "vs minggu lalu" },
    { label: "Avg split", value: rupiah(revenue), icon: BarChart3, delta: "+8.2%", caption: "vs minggu lalu" },
    { label: "Order linked", value: String(orders.length), icon: ClipboardList, delta: "Aktif", caption: "hari ini" },
  ];
    ...
    <div className="min-h-full space-y-4 bg-[#fffaf5] p-3 pb-4 sm:p-5 lg:p-6">
      <section className="rounded-[18px] border border-slate-100 bg-white p-4 shadow-sm sm:rounded-[28px] sm:p-6">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--portal-primary)]">Restaurant payment</p>
            <h1 className="mt-1.5 text-xl font-black text-[#172033] sm:mt-2 sm:text-3xl">Split Bill</h1>
            <p className="mt-1 max-w-3xl text-xs font-bold leading-5 text-slate-500 sm:mt-2 sm:text-sm sm:leading-6">{subIndustryName} - Pecah tagihan transaksi restoran per tamu atau grup pembayaran.</p>
          </div>
