...
import { ClipboardList, RefreshCcw, Search, Store } from "lucide-react";
...
export function CloudKitchenOrderHubLayout({ subIndustryName = "Cloud Kitchen" }: { subIndustryName?: string }) {
  ...
  const headers = ["Invoice", "Channel", "Kitchen", "Fulfillment", "Nilai/ETA", "Jam"];

  const kpis = [
    { label: "Order masuk", value: String(filteredRows.length), icon: ClipboardList, delta: "+15%", caption: "vs minggu lalu" },
    { label: "Channel aktif", value: rupiah(revenue), icon: Store, delta: "+8.2%", caption: "vs minggu lalu" },
      ...
      <section className="rounded-[18px] border border-slate-100 bg-white p-4 shadow-sm sm:rounded-[28px] sm:p-6">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
          <div>
            ...
            <h1 className="mt-1.5 text-xl font-black text-[#172033] sm:mt-2 sm:text-3xl">Order Hub</h1>
            <p className="mt-1 max-w-3xl text-xs font-bold leading-5 text-slate-500 sm:mt-2 sm:text-sm sm:leading-6">{subIndustryName} - Pantau pesanan multi-channel dari POS manual dan channel delivery.</p>
          </div>
