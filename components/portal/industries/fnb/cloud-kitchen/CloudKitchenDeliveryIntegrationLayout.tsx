...
export function CloudKitchenDeliveryIntegrationLayout({ subIndustryName = "Cloud Kitchen" }: { subIndustryName?: string }) {
  ...
  const loadData = async () => {
    ...
    try {
      ...
      setIntegrations(snap.deliveryIntegrations || []);
    } catch (e) {
  ...
  const rows = useMemo(() => {
    return integrations.map((integration) => ({
      ...
      cells: [
        integration.channel,
        integration.status,
        `${integration.menuMapped} menu mapped`,
        integration.notes
      ]
    }));
  }, [integrations]);
  ...
  const headers = ["Channel", "Status", "Menu Mapping", "Catatan"];

  const kpis = [
    { label: "Connected", value: String(filteredRows.length), icon: Truck, delta: "+15%", caption: "vs minggu lalu" },
    { label: "Pending map", value: "2", icon: BarChart3, delta: "+8.2%", caption: "vs minggu lalu" },
    { label: "Integrations", value: String(integrations.length), icon: ClipboardList, delta: "Aktif", caption: "hari ini" },
  ];
    ...
    <div className="min-h-full space-y-4 bg-[#fffaf5] p-3 pb-4 sm:p-5 lg:p-6">
      <section className="rounded-[18px] border border-slate-100 bg-white p-4 shadow-sm sm:rounded-[28px] sm:p-6">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--portal-primary)]">Aggregator</p>
            <h1 className="mt-1.5 text-xl font-black text-[#172033] sm:mt-2 sm:text-3xl">Delivery Integrations</h1>
            <p className="mt-1 max-w-3xl text-xs font-bold leading-5 text-slate-500 sm:mt-2 sm:text-sm sm:leading-6">{subIndustryName} - Status koneksi channel delivery dan readiness integrasi aggregator.</p>
          </div>
