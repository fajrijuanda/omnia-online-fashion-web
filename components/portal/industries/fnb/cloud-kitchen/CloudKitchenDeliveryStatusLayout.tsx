...
import { Truck, RefreshCcw, Search, ClipboardList, BarChart3 } from "lucide-react";
...
export function CloudKitchenDeliveryStatusLayout({ subIndustryName = "Cloud Kitchen" }: { subIndustryName?: string }) {
  ...
  const loadData = async () => {
    ...
    setError("");
    try {
      const snap = await apiFetch<any>("/fnb/operations/snapshot");
      setStatuses(snap.deliveryStatuses || []);
    } catch (e) {
  ...
  const rows = useMemo(() => {
    return statuses.map((status) => ({
      ...
      cells: [
        ...
        status.fulfillment,
        `${status.etaMinutes} menit`,
        new Date(status.createdAt).toLocaleTimeString("id-ID", { hour: "2-digit", minute: "2-digit" })
      ]
    }));
  }, [statuses]);
  ...
  const headers = ["Invoice", "Channel", "Kitchen", "Fulfillment", "ETA", "Jam"];

  const avgEta = Math.round(statuses.reduce((sum, row) => sum + row.etaMinutes, 0) / Math.max(1, statuses.length));

  const kpis = [
    { label: "On delivery", value: String(filteredRows.length), icon: Truck, delta: "+15%", caption: "vs minggu lalu" },
    { label: "Avg ETA", value: `${avgEta} menit`, icon: BarChart3, delta: "+8.2%", caption: "vs minggu lalu" },
    { label: "Delivery", value: String(statuses.length), icon: ClipboardList, delta: "Aktif", caption: "hari ini" },
  ];
    ...
    <div className="min-h-full space-y-4 bg-[#fffaf5] p-3 pb-4 sm:p-5 lg:p-6">
      <section className="rounded-[18px] border border-slate-100 bg-white p-4 shadow-sm sm:rounded-[28px] sm:p-6">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--portal-primary)]">Fulfillment</p>
            <h1 className="mt-1.5 text-xl font-black text-[#172033] sm:mt-2 sm:text-3xl">Delivery Status</h1>
            <p className="mt-1 max-w-3xl text-xs font-bold leading-5 text-slate-500 sm:mt-2 sm:text-sm sm:leading-6">{subIndustryName} - Pantau pickup, driver, ETA, dan status pengiriman pesanan.</p>
          </div>
      ...
      <div className="grid gap-2 lg:gap-4 grid-cols-1 sm:grid-cols-3">
