...
import { Store, RefreshCcw, Search, ClipboardList, BarChart3 } from "lucide-react";
...
export function CloudKitchenMultiBrandLayout({ subIndustryName = "Cloud Kitchen" }: { subIndustryName?: string }) {
  ...
  const loadData = async () => {
    ...
    setError("");
    try {
      setCatalog(await apiFetch<Catalog>("/fnb/pos/catalog?scope=management"));
    } catch (e) {
  ...
  const rows = useMemo(() => {
    return catalog.categories.map((category, index) => ({
      ...
      cells: [
        category.name,
        `${catalog.products.filter((product) => product.categoryId === category.id).length} menu`,
        index % 2 ? "Delivery only" : "Dine-in + delivery",
        index % 2 ? "Sync pending" : "Live"
      ]
    }));
  }, [catalog]);
  ...
  const headers = ["Brand/Kategori", "Menu", "Mode", "Status"];

  const kpis = [
    { label: "Brand aktif", value: String(filteredRows.length), icon: Store, delta: "+15%", caption: "vs minggu lalu" },
    { label: "Menu live", value: String(catalog.products.length), icon: BarChart3, delta: "+8.2%", caption: "vs minggu lalu" },
    { label: "Sync Status", value: "Aktif", icon: ClipboardList, delta: "Normal", caption: "hari ini" },
  ];
    ...
    <div className="min-h-full space-y-4 bg-[#fffaf5] p-3 pb-4 sm:p-5 lg:p-6">
      <section className="rounded-[18px] border border-slate-100 bg-white p-4 shadow-sm sm:rounded-[28px] sm:p-6">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-end xl:justify-between">
          <div>
            <p className="text-xs font-black uppercase tracking-[0.22em] text-[var(--portal-primary)]">Cloud kitchen</p>
            <h1 className="mt-1.5 text-xl font-black text-[#172033] sm:mt-2 sm:text-3xl">Multi-Brand Menu</h1>
            <p className="mt-1 max-w-3xl text-xs font-bold leading-5 text-slate-500 sm:mt-2 sm:text-sm sm:leading-6">{subIndustryName} - Kelola mapping brand, kategori, dan menu untuk dapur multi-brand.</p>
          </div>
