...
const educationOverviewProfiles: Record<string, { caption: string; limit: string; primaryValue: string; primaryLabel: string }> = {
  lms: {
    caption: "Portal E-Learning untuk manajemen mata kuliah, tugas, presensi, dan buku nilai terintegrasi.",
    limit: "multi prodi & kelas",
    primaryValue: "0",
    primaryLabel: "Kelas Aktif"
  },
  kkn: {
    caption: "Manajemen KKN & Fieldwork dengan plotting lokasi, kelompok, logbook harian, dan laporan akhir.",
    limit: "multi wilayah & DPL",
    primaryValue: "0",
    primaryLabel: "Kelompok KKN"
  },
  academic: {
    caption: "Layanan akademik, antrean pengajuan SIPT, repositori dokumen, approval, dan billing kasir.",
    limit: "SLA approval berjenjang",
    primaryValue: "0",
    primaryLabel: "Antrean Request"
  }
};
...
export function AcademicPortal({ activePage, currentTier }: AcademicPortalProps) {
  ...
  const subIndustrySlug = "academic";
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [req, inv, doc, app] = await Promise.all([
        apiFetch<any[]>("/tenant/academic/academic-requests"),
        apiFetch<any[]>("/tenant/academic/billing"),
        apiFetch<any[]>("/tenant/academic/documents"),
        apiFetch<any[]>("/tenant/academic/approvals")
      ]);
      setAcademicRequests(req);
      setInvoices(inv);
      setDocuments(doc);
      setApprovals(app);
    } catch (e) {
  ...
  if (subIndustrySlug === "academic") return <div className="p-6"><AcademicWorkspace moduleKey={activeModuleKey as any} academicRequests={academicRequests} invoices={invoices} documents={documents} approvals={approvals} /></div>;
