...
import { KknWorkspace } from "./KknWorkspace";
import { HigherEducationTier } from "../educationTypes";

interface KknPortalProps {
...
export function KknPortal({ activePage, currentTier }: KknPortalProps) {
  ...
  const load = useCallback(async () => {
    ...
    setLoading(true);
    try {
      const [grp, loc, log, rep] = await Promise.all([
        apiFetch<any[]>("/tenant/kkn/kkn/groups"),
        apiFetch<any[]>("/tenant/kkn/kkn/locations"),
        apiFetch<any[]>("/tenant/kkn/kkn/logbook"),
        apiFetch<any[]>("/tenant/kkn/kkn/reports")
      ]);
      setKknGroups(grp);
      setKknLocations(loc);
      setKknLogbook(log);
  ...
  if (subIndustrySlug === "kkn") return <div className="p-6"><KknWorkspace moduleKey={activeModuleKey as any} kknGroups={kknGroups} kknLocations={kknLocations} kknLogbook={kknLogbook} kknReports={kknReports} /></div>;
